...
// filenameUnsafe matches characters not allowed in snippet filenames.
// Each character is replaced individually (no + quantifier) to preserve
// identical output to the prior strings.ReplaceAll chain.
...
type Manager struct {
	...
	filePaths map[string]string   // key: snippet ID, value: absolute file path
...
func (m *Manager) LoadAll() error {
	...
	for _, fp := range files {
		...
		if err != nil {
			slog.Warn("failed to read file", "path", fp, "error", err)
			continue
		}

		snippet, err := ParseFrontmatter(content)
		if err != nil {
			slog.Warn("failed to parse file", "path", fp, "error", err)
			continue
		}

		if err := snippet.Validate(); err != nil {
			slog.Warn("invalid snippet in file", "path", fp, "error", err)
			continue
		}

		m.snippets[snippet.ID] = snippet
		m.filePaths[snippet.ID] = fp
	}
...
func (m *Manager) Save(snippet *Snippet) error {
	...
	// Use cached file path if updating, otherwise generate new filename
	filePath, exists := m.filePaths[snippet.ID]
	if !exists {
	...
	m.snippets[snippet.ID] = snippet
	m.filePaths[snippet.ID] = filePath
...
func (m *Manager) Delete(id string) error {
	...
	// Delete the file using the cached path
	if fp, ok := m.filePaths[id]; ok {
		if err := m.storage.DeleteFile(fp); err != nil {
...
func generateFilename(snippet *Snippet) string {
	title := filenameUnsafe.ReplaceAllString(snippet.Title, "_")
	return fmt.Sprintf("%s_%s.md", title, snippet.UpdatedAt.Format("20060102_150405"))
}
