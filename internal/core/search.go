...
// filterCandidates returns the subset of snippets matching all tag and language filters.
func filterCandidates(snippets map[string]*Snippet, opts SearchOptions) map[string]*Snippet {
...
// fuzzySearch performs fuzzy matching on snippet titles.
// Returns matched results and a set of matched snippet IDs for exclusion.
func fuzzySearch(candidates map[string]*Snippet, query string) ([]*SearchResult, map[string]bool) {
	titles := make([]string, 0, len(candidates))
	snippetByTitle := make(map[string]*Snippet)
	for _, snippet := range candidates {
		titles = append(titles, snippet.Title)
		snippetByTitle[snippet.Title] = snippet
	}
	...
	for _, match := range matches {
		snippet := snippetByTitle[match.Str]
		results = append(results, &SearchResult{
			Snippet: copySnippet(snippet),
			Score:   match.Score,
		})
		matched[snippet.ID] = true
	}

	return results, matched
}

// substringSearch performs substring matching on tags, description, and body
// for candidates not already matched by fuzzySearch.
func substringSearch(candidates map[string]*Snippet, query string, exclude map[string]bool) []*SearchResult {
	queryLower := strings.ToLower(query)
	results := make([]*SearchResult, 0)

	for _, snippet := range candidates {
		if exclude[snippet.ID] {
			continue
		}
...
func (m *Manager) SearchWithFilters(opts SearchOptions) []*SearchResult {
	...
	candidates := filterCandidates(m.snippets, opts)

	if opts.Query == "" {
		results := make([]*SearchResult, 0, len(candidates))
		for _, snippet := range candidates {
			results = append(results, &SearchResult{Snippet: copySnippet(snippet), Score: 0})
		}
		applySort(results, opts, false)
		return results
	}

	fuzzyResults, matched := fuzzySearch(candidates, opts.Query)
	subResults := substringSearch(candidates, opts.Query, matched)

	results := append(fuzzyResults, subResults...)
